...
%% LOW LEVEL EMPC  
xsp = [zeros(N,1), 1.5*10^(-3)*ones(N,1), 20*ones(N,1), zeros(N,1)]';
...
plot(0:Nsim, x_cl(1,:), 'LineWidth', 1.2); title('Weight in kg m^{-2}');
subplot(2,4,2);hold on;
plot(0:Nsim, x_cl(2,:), 'LineWidth', 1.2); hold on;
plot(0:Nsim, 1.5*10^(-3)*ones(Nsim+1,1),'--', 'LineWidth', 1.2); title('Indoor CO2 in kg m^{-3}');
subplot(2,4,3);hold on;
plot(0:Nsim, x_cl(3,:), 'LineWidth', 1.2); hold on;
plot(0:Nsim, 20*ones(Nsim+1,1), '--', 'LineWidth', 1.2); title('Air Temp in  ^oC');
subplot(2,4,4);hold on;
plot(0:Nsim, x_cl(4,:), 'LineWidth', 1.2); title('RH C_{H2O} in %');
subplot(2,4,5);
plot(0:Nsim-1, u_cl(1,:), 'LineWidth', 1.2); title('CO2 in mg*m^{-2}*s^{-1}');
subplot(2,4,6);
plot(0:Nsim-1, u_cl(2,:), 'LineWidth', 1.2); title('vent. rate in mm s^{-1}');
subplot(2,4,7);
plot(0:Nsim-1, u_cl(3,:), 'LineWidth', 1.2); title(' heat supply in W*m^{-2}');
% u1: Supply rate of CO2 in mg*m^{-2}*s^{-1}
% u2: Ventilation rate through the vents in mm s^{-1}
% u3: Energy supply by the heating system in W*m^{-2}
%%
